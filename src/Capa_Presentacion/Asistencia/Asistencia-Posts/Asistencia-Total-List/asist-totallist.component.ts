...
@Component({
    selector:'app-totalasist-list',
    templateUrl:'./asist-totallist.component.html',
    styleUrls:['./asist-totallist.component.css'],
    encapsulation: ViewEncapsulation.None

})
export class AsistTotalListComponent implements OnInit{
  ...
  isLoading=false;
   posts :Asistencia[]=[
    ...
    private PostsSub: Subscription;
     //table data
     displayedColumns: string[] = ['Nombre_Empleado','Apellido_Empleado', 'Total'];
    ...
    ngOnInit(){
      this.isLoading=true;
      console.log();
      console.log("El  modo ingresada es: "+this.postsService.PostMode);

      console.log("Modo para todos? :"+this.postsService.allmode);

      if(this.postsService.allmode==true){

        if(this.postsService.PostMode==3){
      ...
      else
      {
        ...
        //para buscar solo de un empleado
      
      if(this.postsService.PostMode==3){
        this.postsService.getOneTotalatraso();
        this. mostrarTotalsAsis();
        ...
        this.postsService.getOneTotalFaltas();
        this. mostrarTotalsAsis();

      }
      else if(this.postsService.PostMode==7){
        this.postsService.getOneTotalHE();
        this. mostrarTotalsAsis();
        ...
        mostrarTotalsAsis(){
          ...
          this.PostsSub= this.postsService.getPostUpdateListenerfour().subscribe
                ((fposts:AsistenciaFour[])=>{
                    ...
                    console.log("length de todo is: ",fposts.length);
                    if(fposts.length==0){
...
export(){
  ...
  this.matTableExporter.exportTable('xlsx',{fileName:'ResumenTotal'});
